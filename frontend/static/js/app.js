        ...
        return fetch("/api/compare", {
          ...
          .then(cmp => {
            if (cmp.status === "success") {
              ...
              fetchAISummary(category, bestRecommendation.material_name, currentMaterial, cmp.comparison.co2_savings_kg, cmp.comparison.cost_difference_inr);
            }
          });
      }
    })
...
function displayResults(recs) {
  ...
  recs.forEach((r, i) => {
    ...
    card.innerHTML = `
      ...
      <div id="ai-panel-${i}" class="ai-glass-panel glowing-border" style="display: ${i === 0 ? 'block' : 'none'};">
        <div class="ai-header">
        ...
        <div class="ai-body typing-indicator" id="ai-body-${i}">Analyzing sustainability profile...</div>
      </div>
    `;
  ...
  fetchAIExplanation(category, weight, fragility, currentMaterial, recs);
  fetchAICompliance(category, best.material_name);
}

function fetchAIExplanation(category, weight, fragility, currentMaterial, recs) {
  const best = recs[0];
  const alternatives = recs.slice(1).map(r => r.material_name);
  
  fetch('/api/ai/explain', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      category, weight, fragility, 
      recommended_material: best.material_name,
      current_material: currentMaterial,
  ...
  .then(data => {
    if (data.status === 'success') {
      const panel = document.getElementById('ai-body-0');
      panel.classList.remove('typing-indicator');
      document.getElementById('ai-panel-0').classList.remove('glowing-border');
      
      const exp = data.explanation;
      panel.innerHTML = `
        <div style="margin-bottom: 8px;"><strong>Reasoning:</strong> ${exp.reasoning}</div>
        <div style="margin-bottom: 8px;"><strong>Carbon Impact:</strong> ${exp.carbon_savings_explanation}</div>
        <div><strong>Trade-offs:</strong> ${exp.tradeoffs}</div>
      `;
    }
  }).catch(console.error);
}

function fetchAICompliance(category, recommended_material) {
  fetch('/api/ai/compliance', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ category, recommended_material })
  })
  .then(res => res.json())
  .then(data => {
    if (data.status === 'success') {
      const cmp = data.compliance;
      const panel = document.getElementById('ai-panel-0');
      const badgesHtml = cmp.compliance_badges.map(b => `<span class="ai-badge compliant">✓ ${b}</span>`).join('');
      const warningsHtml = cmp.warnings.map(w => `<span class="ai-badge warning">! ${w}</span>`).join('');
      
      const complianceDiv = document.createElement('div');
      complianceDiv.style.marginTop = '12px';
      complianceDiv.innerHTML = `<strong>Compliance:</strong><br>${badgesHtml} ${warningsHtml}`;
      panel.appendChild(complianceDiv);
    }
  }).catch(console.error);
}

function fetchAISummary(category, recommended_material, current_material, co2_savings, cost_savings) {
  const summaryContainer = document.getElementById('aiSummaryContainer');
  summaryContainer.style.display = 'block';
  summaryContainer.innerHTML = `
    <div class="ai-glass-panel glowing-border" style="margin-bottom: 24px;">
      <div class="ai-header">
      ...
      <div class="ai-body typing-indicator" style="font-size: 1.1rem; font-weight: 500;">Drafting strategic insight...</div>
    </div>
  `;
  ...
  fetch('/api/ai/summary', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ category, recommended_material, current_material, co2_savings, cost_savings })
  })
  .then(res => res.json())
  .then(data => {
    if (data.status === 'success') {
      const panel = summaryContainer.querySelector('.ai-glass-panel');
      panel.classList.remove('glowing-border');
      const body = summaryContainer.querySelector('.ai-body');
      body.classList.remove('typing-indicator');
      body.innerHTML = `"${data.summary.summary}"`;
    }
  }).catch(console.error);
}
