...
from llm_engine.services.explanation_service import ExplanationService
import traceback
...
logger.info("Recommendation engine and AI service ready.")
...
def ai_explain():
    try:
        data = request.get_json()
        result = explanation_service.explain_sustainability(
            category=data.get('category'),
            weight=data.get('weight'),
            fragility=data.get('fragility'),
            recommended_material=data.get('recommended_material'),
            current_material=data.get('current_material'),
            suitability_score=data.get('suitability_score'),
            eco_score=data.get('eco_score')
        )
        
        alt_result = explanation_service.analyze_alternatives(
            category=data.get('category'),
            weight=data.get('weight'),
            alternatives=data.get('alternatives', [])
        )
        result['alternatives_analysis'] = alt_result.get('alternatives', [])
        
        return jsonify({'status': 'success', 'explanation': result}), 200
    except Exception as e:
...
def ai_summary():
    try:
        data = request.get_json()
        result = explanation_service.generate_executive_summary(
            category=data.get('category'),
            recommended_material=data.get('recommended_material'),
            current_material=data.get('current_material'),
            co2_savings=data.get('co2_savings'),
            cost_savings=data.get('cost_savings')
        )
        return jsonify({'status': 'success', 'summary': result}), 200
    except Exception as e:
        logger.error(f"AI Summary error: {traceback.format_exc()}")
        return jsonify({'status': 'error', 'message': 'Internal server error'}), 500

@app.route('/api/ai/compliance', methods=['POST'])
@require_api_key
def ai_compliance():
    try:
        data = request.get_json()
        result = explanation_service.analyze_compliance(
            category=data.get('category'),
            recommended_material=data.get('recommended_material')
        )
        return jsonify({'status': 'success', 'compliance': result}), 200
    except Exception as e:
        logger.error(f"AI Compliance error: {traceback.format_exc()}")
        return jsonify({'status': 'error', 'message': 'Internal server error'}), 500
