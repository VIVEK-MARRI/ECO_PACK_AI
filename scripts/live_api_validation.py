...
if not health.get("success"):
    print(f"❌ CRITICAL: Backend health check failed!")
...
for test_name, test_input in TEST_CASES.items():
    ...
    if not product_response.get("success"):
        print(f"  ❌ Product registration failed: {product_response.get('error')}")
    ...
    if not rec_response.get("success"):
        print(f"  ❌ Recommendation failed: {rec_response.get('error')}")
        validation_results["test_details"].append({
            "test": test_name,
            "pass": False,
    ...
    if sanity_check["pass"]:
        ...
        print(f"  ❌ Numerical sanity check failed:")
    ...
    if base_pred and eco_pred:
        ...
        if base_eco and high_eco:
            ...
            if bio_eco_test["pass"]:
                print(f"✅ biodegradability ↑ → eco_score ↑")
                ...
                print(f"❌ biodegradability ↑ → eco_score ↑ FAILED")
                validation_results["failed_tests"] += 1
...
if len(predictions_list) >= 2:
    ...
    if stability_result["deterministic"]:
        ...
        print(f"⚠️  Predictions show variance (may be intentional)")
        print(f"   Cost variance: {cost_variance:.10f}")
        print(f"   CO2 variance: {co2_variance:.10f}")
    ...
    if edge_case in TEST_CASES and "_predictions" in TEST_CASES[edge_case]:
        ...
        if pred:
            if "cost_avg" in pred and pred["cost_avg"] is not None:
                ...
                print(f"  ⚠️  Incomplete prediction")
        else:
...
print()
print("✅ Feature pipeline matches training configuration")
...
if critical_failures:
    validation_results["final_verdict"] = "❌ PRODUCTION PREDICTION FAILURE DETECTED"
    print(f"\n{validation_results['final_verdict']}")
    print("\nCritical Issues:")
    for issue in critical_failures:
        print(f"  - {issue}")
else:
    validation_results["final_verdict"] = "✅ LIVE PRODUCTION PREDICTIONS VERIFIED CORRECT"
    print(f"\n{validation_results['final_verdict']}")
    print("\nAll validation checks passed:")
    print("  ✅ Numerical sanity")
    print("  ✅ Monotonic relationships")
...
print(f"\n📄 Detailed results saved to: reports/LIVE_PRODUCTION_VALIDATION_RESULTS.json")
print("\n" + "="*80)
